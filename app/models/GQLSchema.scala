  ...
  val query: ObjectType[Backend, Unit] = ObjectType(
    ...
    fields[Backend, Unit](
      ...
      Field(
        ...
        arguments = optQueryString :: entityNames :: facetSearchFilters :: pageArg :: Nil,
        resolve = ctx => {
          val queryString = ctx.arg(optQueryString).getOrElse("")
          val entities = ctx.arg(entityNames).getOrElse(Seq.empty)
          ctx.ctx.searchFacets(queryString, ctx.arg(pageArg), entities, ctx.arg(facetSearchFilters))
        }
      ),
      Field(
        "mapIds",
